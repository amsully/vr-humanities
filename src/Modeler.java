...
class Modeler extends JFrame
        ...
        /*
         * This defines the objModel class, which takes care of loading a
         * triangular mesh from an obj file, estimating per vertex average
         * normal, and displaying the mesh.
         */
        class objModel {
                ...
                public objModel(String filename) {
                        ...
                        System.out.println("Read " + input_verts.size() + " vertices and " + input_faces.size()
                                        + " faces.");
        ...
        private objModel example_model = new objModel("statue_one.obj");

        float mat_ambient[] = {1f, .1f, .8f, 1f};
        float mat_diffuse[] = { .2f, .2f, .6f, 1f };
        float mat_specular[] = { .8f, .8f, .8f, 1f };
        float mat_shininess[] = { 128 }; //range[0,128]
        float mat_emission[] = { 0.0f, 0.25f, 0.0f, 1.0f };
        
        private float example_rotateT = 0.f;
        private float levelOne = 0.f;
...
//        private float xmax = 1f, ymax = 1f, zmax = 1f;
        ...
        public Modeler() {
                ...
                animator = new FPSAnimator(canvas, 30); // create a 30 fps
                                                        // animator
        ...
        /*
         * computes optimal transformation parameters for OpenGL rendering. this
         * is based on an estimate of the scene's bounding box
         */
        void initViewParameters() {
                roth = rotv = 0;

                float ball_r = (float) Math.sqrt((xmax - xmin) * (xmax - xmin) + (ymax - ymin) * (ymax - ymin)
